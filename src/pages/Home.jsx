...
const CURRENT_ROLE = (() => {
  ...
  return r ? { id: r.id, company: r.company, title: r.title, period: r.period, tags: ROLE_TAGS[r.id] || [], current: true } : null
})()

// Prior roles, most-recent-first (resume order minus the current role), plus earlier-career.
const PRIOR_ROLES = [
  ...EXPERIENCE.filter(r => !r.current).map(r => ({
    id: r.id, company: r.company, title: r.title, period: r.period, tags: ROLE_TAGS[r.id] || [],
  })),
  EARLIER_CAREER_CARD,
]
...
function RoleTags({ tags, isCurrent }) {
  return (
    <div className="card-tags">
      {tags.map((tag, i) => (
        <span key={i} className={`tag${isCurrent ? ' tag-current' : ''}`}>{tag}</span>
      ))}
    </div>
  )
}
        ...
        {ALL_ROLES.map(role => (
          ...
          >
            <div className="stacked-period">
              {role.current ? '2022 → present' : role.period}
            </div>
